...
type SimConfig struct {
	...
	MemtableFlushTimeoutSec float64 `json:"memtableFlushTimeoutSec"` // Max time before force flush (custom, not standard RocksDB)
...
func DefaultConfig() SimConfig {
	return SimConfig{
		...
		MemtableFlushTimeoutSec:          300.0, // 5 minutes timeout (custom - force flush old data)
...
func ThreeLevelConfig() SimConfig {
	return SimConfig{
		...
		MemtableFlushTimeoutSec:          60.0,  // 1 minute timeout (shorter for testing)
...
func (c *SimConfig) Validate() error {
	...
	if c.MemtableFlushTimeoutSec < 0 {
		return ErrInvalidConfig("memtableFlushTimeoutSec must be >= 0")
	}
